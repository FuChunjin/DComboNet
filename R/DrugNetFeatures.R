...
structure.sim <- function(CDK_FP=NULL,
  ...
  if (requireNamespace("compiler")){
    ...
    if((!is.null(CDK_FP))& (!is.null(pubchemFP))& (!is.null(MACCS_FP))){
      CDK_FP1 = read.csv(paste0(load_dir,'/data/fingerprints/fingerprints.csv'),stringsAsFactors = F)
      CDK_FP = unique(rbind(CDK_FP1,CDK_FP))
      pubchemFP1=read.csv(paste0(load_dir,'/data/fingerprints/pubchem_fingerprints.csv'),stringsAsFactors = F)
      pubchemFP = unique(rbind(pubchemFP1,pubchemFP))
      MACCS_FP1=read.csv(paste0(load_dir,'/data/fingerprints/MACCS_fingerprints.csv'),stringsAsFactors = F)
      MACCS_FP = unique(rbind(MACCS_FP1,MACCS_FP))

    }else{
      CDK_FP = read.csv(paste0(load_dir,'/data/fingerprints/fingerprints.csv'),stringsAsFactors = F)
      pubchemFP=read.csv(paste0(load_dir,'/data/fingerprints/pubchem_fingerprints.csv'),stringsAsFactors = F)
      MACCS_FP=read.csv(paste0(load_dir,'/data/fingerprints/MACCS_fingerprints.csv'),stringsAsFactors = F)
    }
...
SE.sim <- function(druglist=NULL, load_dir){
  if (requireNamespace("compiler")){
    ...
    if(!is.null(druglist)){
      ...
      drug_se = read.csv(paste0(load_dir,'data/drug_se.txt'),sep='\t',header=T, stringsAsFactors = F)
    }
...
DrugNetFeature <- function(druglist = NULL,
  ...
  if(requireNamespace(c("reshape2","compiler"))){
    ...
    drugpair_base = read.csv(paste0(load_dir,'data/drugpair_base.csv'),stringsAsFactors = F)
    drugpair_P = drugpair_base[drugpair_base$TAG == 'P',]
    drugpair_P$label = paste0(drugpair_P$A, '_',drugpair_P$B)
    drugpair_N = drugpair_base[drugpair_base$TAG == 'N',]
    druglib = read.csv(paste0(load_dir,'data/druglist.csv'), header = T, stringsAsFactors = F)

    # if(is.null(drugpair)){
    #
    #   drugpair = drugpair_base
    # }else{
    #   drugpair$TAG = 'N'
    #   names(drugpair) = names(drugpair_base)
    #   drugpair = rbind(drugpair_base,drugpair)
    # }
    if(!is.null(druglist)){

      druglib_sum = union(druglist[,1], druglib[,1])
      ...
      print(paste0('Basic drug number: ',nrow(druglib)))
      print(paste0('Input drug number: ',nrow(druglist)))

      if(length(intersect(druglib[,1],druglist[,1]))!=0){
        print(paste0('Overlaping drug number: ',length(intersect(druglib[,1],druglist[,1]))))
      }else{
    ...
    # features1 = merge(features,drugpair, by=c('A','B'))
    # drugpair = drugpair[c(2,1,3)]
    # names(drugpair) = c('A','B','TAG')
    # features2 = merge(features,drugpair, by=c('A','B'))
