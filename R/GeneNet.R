...
cancerGene = function(cellline,
  ...
  load(paste0(load_dir, 'data/celllineExpr/CCLEexprMatrix.Rdata'))
  ...
  if(cellline %in% colnames(ccleexpr)){
    ...
    write.table(cell_genelist, paste0(load_dir,'data/cellline_genes/',cellline,'_genelist.txt', sep = ''), sep = '\t', col.names = T, row.names = F)
  }else{
...
geneNet <- function(dt = NULL,
  ...
  if (requireNamespace(c('igraph','dnet'))){
    ...
    PPI_table <- read.csv(paste0(load_dir,"data/network/PPI_inbio_score.csv"), header = T, stringsAsFactors = F)
    ...
    if(is.null(cellline)){
      ...
      if(!(cellline %in% celllineDEG_lib[,1]) && is.null(dDEG)){
        ...
        if((cellline %in% cellline_lib[,1]) && is.null(cancer_gene)){
          ...
          cancer_gene = read.csv(paste0(load_dir,'data/cellline_genes/',cellline,'_genelist.csv', sep = ''),header = F, stringsAsFactors = F)
          cancer_gene = union(cancer_gene[,1],unique(union(dt$Target,dg$Target)))
