...
genepathwayAdj <- function(drugAdj,
  ...
  N = nrow(pathwayadj)
  gp = read.table(paste0(load_dir,'pathway/KEGG_ID_gene.txt'),sep='\t',header = T,stringsAsFactors = F)
  names(gp) = c('KEGG_ID','pathway_name','Gene')

  for(i in 1:N){
    pathway <- pathways[i]
    gene <- gp[which(gp$KEGG_ID == pathway),]$Gene
