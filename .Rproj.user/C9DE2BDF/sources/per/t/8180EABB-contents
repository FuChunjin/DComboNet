...
network_extract <- function(drugseed,
  ...
  if (requireNamespace('igraph')){
    drugtarget_lib = read.csv(paste0(load_dir, '/data/drugtarget.csv'), sep = ',',header = T, stringsAsFactors = F)

    if(is.null(drugtarget)){
      drugtarget = drugtarget_lib
    }else{
      names(drugtarget) = names(drugtarget_lib)
      drugtarget = unique(rbind(drugtarget_lib, drugtarget))
    }
...
network_visualization <- function(drugseed,
  ...
  if (requireNamespace('igraph','visNetwork')){
